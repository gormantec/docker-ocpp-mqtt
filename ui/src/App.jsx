...
export default function App() {
  ...
  // Total live power across all connectors (from meter_values)
          ...
          <>
            <div className="summary-cards">
              <div className="summary-card">
                <div className={"summary-value" + (totalPower > 0 ? ' text-green' : '')}>
                  {totalPower > 0 ? Math.round(totalPower) + 'W' : '0W'}
                </div>
                <div className="summary-label">Charging Power</div>
              </div>
              <div className="summary-card">
                <div className={"summary-value" + (solar.grid_export > 2000 ? ' text-green' : solar.grid_import > 0 ? ' text-red' : '')}>
                  {solar.grid_export || 0}W
                </div>
                <div className="summary-label">Grid Export</div>
              </div>
              <div className="summary-card">
                <div className={"summary-value" + ((solar.battery_soc || 0) > 50 ? ' text-green' : ' text-warn')}>
                  {solar.battery_soc || 0}%
                </div>
              ...
              <div className="summary-card">
                <div className="summary-value">
                  {(() => {
                    ...
                    return '⏱ ' + (schedCfg.periods || DEFAULT_PERIODS).length + 'w';
                  })()}
                </div>
            ...
            {selectedCp && (
              <div className="card">
                <div className="card-header">
                  ...
                  <span className={'badge ' + (selectedCp.status === 'Charging' ? 'badge-on' : selectedCp.status === 'Available' ? 'badge-info' : 'badge-warn')}>
                ...
                <div className="card-body">
                  <div className="table-wrap">
                    <table className="data-table">
                      <thead>
                        <tr><th>Connector</th><th>Status</th><th>Power</th><th>Energy</th><th>Last Update</th></tr>
                      </thead>
                      <tbody>
                        {Object.entries(selectedCp.physical_status || {}).map(([connId, status]) => {
                          ...
                          return (
                            <tr key={connId}>
                              <td>Connector {connId}</td>
                              <td><span className={'badge ' + (status === 'Charging' ? 'badge-on' : status === 'Available' ? 'badge-info' : 'badge-off')}>{status}</span></td>
                              <td>{mv.power != null ? Math.round(mv.power) + 'W' : String.fromCharCode(8212)}</td>
                              <td>{mv.energy != null ? (mv.energy / 1000).toFixed(1) + 'kWh' : String.fromCharCode(8212)}</td>
                              <td className="date-cell">{mv.timestamp ? new Date(mv.timestamp).toLocaleTimeString() : String.fromCharCode(8212)}</td>
                            </tr>
                          );
                        })}
                        {Object.keys(selectedCp.physical_status || {}).length === 0 && (
                          <tr><td colSpan={5} style={{textAlign: 'center', color: '#95a5a6'}}>No connectors active</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <div className="empty-state">
                    ...
                    <p className="hint">Configure your EV charger to connect at:<br/>
            ...
            <div className="card">
              ...
              <div className="card-body">
                {!effectiveCpId ? (
                  ...
                  <>
                    {scheduleMsg && (
                      <div className={'alert ' + (scheduleMsg.type === 'success' ? 'alert-success' : 'alert-error')} style={{ marginBottom: 16 }}>
                        {scheduleMsg.text}
                      </div>
                    )}
                    <div className="toggle-group" style={{
                      display: 'inline-flex', borderRadius: 4, width: '100%',
                    ...
                    <div className="hint" style={{ fontSize: 12, color: '#95a5a6', marginTop: 8 }}>
                      ...
                      {(() => {
                        const cfg = schedule[effectiveCpId] || {};
                        const periods = cfg.mode === 'auto' ? (cfg.periods || DEFAULT_PERIODS) : null;
                        return periods ? ' - ' + periods.map(p => p.start_hour + ':00→' + p.limit_watts + 'W').join(', ') : '';
                      })()}
                    </div>
                  </>
                )}
            ...
            <div className="card">
              ...
              <div className="card-body" style={{ maxHeight: 280, overflowY: 'auto' }}>
                {(!data.recent_events || data.recent_events.length === 0) ? (
                  <div className="empty-state"><p>No events yet. Waiting for charge point activity...</p></div>
                ) : (
                  <div className="table-wrap">
                    <table className="data-table">
                      <thead><tr><th>Time</th><th>Charge Point</th><th>Event</th><th>Details</th></tr></thead>
                      <tbody>
                        {data.recent_events
                          ...
                          <tr key={i}>
                            ...
                            <td><span className={'badge ' + getEventBadge(ev.type)}>{ev.type}</span></td>
                            <td className="mono-cell" style={{maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis'}}>{ev.summary || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="card">
              <div className="card-header"><h3>Bridge Info</h3></div>
              <div className="card-body">
                <div className="info-grid">
                  ...
                  <div className="info-item">
                    ...
                    <span className="info-value">{Math.floor((data.uptime_seconds || 0) / 60)}m</span>
                  </div>
                  <div className="info-item">
                    <span className="info-label">Connected CPs</span>
                    <span className={'info-value ' + (connectedCps.length > 0 ? 'text-green' : 'text-red')}>{connectedCps.length}</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="card-body">
                  ...
                  <div style={{marginBottom: 16, padding: '12px', background: '#1a2332', borderRadius: 4, border: '1px solid #3a4552'}}>
                    <label style={{display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 12}}>
                      <input type="checkbox" checked={editSolarSmart} onChange={(e) => setEditSolarSmart(e.target.checked)}
                    ...
                    {editSolarSmart && (
                      <div style={{display: 'flex', gap: 16}}>
                        <div style={{flex: 1}}>
                          <label style={{fontSize: 11, color: '#95a5a6', display: 'block', marginBottom: 3}}>Off-Peak Start</label>
                          <input type="number" min={0} max={23} value={editOffPeakStart}
                            onChange={(e) => setEditOffPeakStart(parseInt(e.target.value) || 0)}
                            style={{width: '100%', padding: '6px 8px', background: '#0d141e', border: '1px solid #3a4552', color: '#d5dbdb', borderRadius: 3}} />
                        </div>
                        <div style={{flex: 1}}>
                          <label style={{fontSize: 11, color: '#95a5a6', display: 'block', marginBottom: 3}}>Off-Peak End</label>
                          <input type="number" min={0} max={23} value={editOffPeakEnd}
                  ...
                  <p className="hint" style={{marginBottom: 16, fontSize: 13, color: '#95a5a6'}}>
                    Each period sets a power limit starting at a given hour. Schedule repeats <strong>daily</strong>.
                  </p>
                  {editPeriods.map((p, i) => (
                    <div key={i} style={{marginBottom: 8, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 12, backgroundColor: '#1a2332', borderRadius: 4}}>
                      <div style={{flex: 1}}>
                        <label style={{fontSize: 11, color: '#95a5a6', display: 'block'}}>Start Hour</label>
                        <input type="number" min={0} max={23} value={p.start_hour}
                          onChange={(e) => {
                      ...
                      <div style={{flex: 2}}>
                        <label style={{fontSize: 11, color: '#95a5a6', display: 'block'}}>Limit (Watts)</label>
                        <input type="number" min={0} max={50000} step={100} value={p.limit_watts}
                          onChange={(e) => {
                            const next = [...editPeriods];
                            next[i] = {...next[i], limit_watts: parseInt(e.target.value) || 0};
                            setEditPeriods(next);
                          }}
                          style={{width: '100%', padding: '6px 8px', background: '#0d141e', border: '1px solid #3a4552', color: '#d5dbdb', borderRadius: 3}} />
                      </div>
                      <button className="btn btn-secondary" style={{padding: '4px 8px', fontSize: 12}}
                        ...
                        onClick={() => { if (editPeriods.length > 1) setEditPeriods(editPeriods.filter((_, idx) => idx !== i)); }}>✕</button>
                    </div>
                  ))}
                  <button className="btn btn-secondary" style={{marginBottom: 16, width: '100%'}}
                    onClick={() => setEditPeriods([...editPeriods, { start_hour: 0, limit_watts: 4800 }])}>+ Add Period</button>
                  <div style={{display: 'flex', gap: 8}}>
                    <button className="btn btn-secondary" style={{flex: 1}} onClick={() => setShowConfig(false)}>Cancel</button>
                    <button className="btn btn-primary" style={{flex: 1}} disabled={schedulePending}
                      onClick={async () => {
                        setSchedulePending(true);
                        try {
                          const res = await fetch(BASE + 'schedule', {
                            ...
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                              cp_id: effectiveCpId, timezone: editTimezone,
                              periods: editPeriods.sort((a, b) => a.start_hour - b.start_hour),
                              solar_smart: editSolarSmart, off_peak_start_hour: editOffPeakStart,
                              off_peak_end_hour: editOffPeakEnd,
                            }),
                          });
                          if (res.ok) {
                            setScheduleMsg({ type: 'success', text: 'Schedule updated' });
                            setShowConfig(false);
                            const schedRes = await fetch(BASE + 'schedule');
                            if (schedRes.ok) {
                              const schedJson = await schedRes.json();
                            ...
                            setScheduleMsg({ type: 'error', text: err.error || 'Update failed' });
                          }
                        } catch (e) {
